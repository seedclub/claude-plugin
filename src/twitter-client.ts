...
/**
 * Detect all Chrome/Chromium profiles on macOS, sorted by most recently used.
 * Returns profile names like "Default", "Profile 1", "Profile 2", etc.
 */
function detectChromeProfiles(): string[] {
  ...
  for (const chromeDir of chromeDirs) {
    ...
    try {
      ...
      if (existsSync(defaultCookies)) {
        profiles.push({
          name: 'Default',
          mtimeMs: statSync(defaultCookies).mtimeMs,
        });
      }
      ...
      for (const name of entries) {
        ...
        if (existsSync(cookiesPath)) {
          profiles.push({
            name,
            mtimeMs: statSync(cookiesPath).mtimeMs,
          });
        }
      }
    } catch {
  ...
  // Sort by most recently used (newest first)
...
/**
 * Clear the cached Twitter client (useful for re-authentication)
 */
...
/**
 * Resolve Twitter credentials from browsers or environment variables.
 * Order: Safari -> Chrome (all profiles) -> Firefox
 *
 * For Chrome, automatically tries all detected profiles in order of most recently used.
 *
 * Environment variables:
 * - CHROME_PROFILE: Chrome profile name (e.g., "Profile 3") to use a specific profile
 * - FIREFOX_PROFILE: Firefox profile name if not using default
 */
async function resolveTwitterCredentials(): Promise<{
  ...
  const safariResult = await resolveCredentials({
    cookieSource: ['safari'] as CookieSource[],
  });
  ...
  // Try Chrome with explicit profile if set, otherwise try all profiles
  ...
  if (explicitChromeProfile) {
    ...
    // Auto-detect and try all Chrome profiles
    ...
    for (const profile of profiles) {
      ...
      // Only add warnings if this is the last profile we try
      if (chromeResult.cookies.authToken && chromeResult.cookies.ct0) {
        return { cookies: chromeResult.cookies, warnings: allWarnings };
      }
    }
  ...
  // Return empty cookies with all accumulated warnings
  return {
...
/**
 * Get or create a TwitterClient instance.
 * Throws TwitterClientError if credentials cannot be resolved.
 */
export async function getTwitterClient(): Promise<TwitterClient> {
  ...
  if (!cookies.authToken || !cookies.ct0) {
    ...
    // Attach warnings for context
    (error as TwitterClientError & { warnings?: string[] }).warnings = warnings;
    throw error;
  }
...
/**
 * Check if Twitter credentials are available and valid.
 * Verifies by making a test API call to get the current user.
 */
export async function checkTwitterCredentials(): Promise<CredentialCheckResult> {
  ...
  // Try to get the current user to verify credentials work
  try {
...
/**
 * Get the cached cookies source (for display purposes)
 */
export function getCachedCredentialSource(): string | null {
  return cachedCookies?.source ?? null;
}
